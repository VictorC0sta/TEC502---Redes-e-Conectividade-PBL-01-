...
def get_historico(segundos=30):
    try:
        r = requests.get(
            f"{SERVER_URL}/historico?segundos={segundos}&limite=600",
            timeout=2
        )
        d = r.json()
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._sensores_status     = {}   # sid -> dict com tipo/valor/cor/icone — fixo
        self._periodo_var         = tk.StringVar(value="30s")
        self._ultimo_historico    = []
    ...
    def _build_ui(self):
        ...
        # Atualiza aba de histórico ao clicar nela
    ...
    def _build_left(self, parent):
        ...
        # Frame fixo para status — NÃO é recriado, só os labels internos mudam
        self.status_frame = tk.Frame(left, bg=BG_PANEL)
        self.status_frame.pack(fill="x", padx=10)
        # Dicionário de widgets de valor por sensor
        self._status_widgets = {}   # sid -> Label do valor
        ...
        tk.Button(
            ...
            font=("Courier New", 9, "bold"),
            bg=COR_DANGER, fg="#ffffff",
            relief="flat", padx=10, pady=8, cursor="hand2",
            activebackground="#c0414b", activeforeground="#ffffff",
            command=lambda: threading.Thread(
                target=ativar_manual, args=("alarme",), daemon=True
            ).start()
        ...
        tk.Button(
            ...
            command=lambda: threading.Thread(
                target=ativar_manual, args=("resfriamento",), daemon=True
            ).start()
        ).pack(fill="x")
    ...
    def _build_center(self, parent):
        ...
        tk.Label(
            center, text="HISTÓRICO EM TEMPO REAL",
            font=("Courier New", 9, "bold"),
            bg=BG_ROOT, fg=TX_MUTED
        ).pack(anchor="w", padx=16, pady=(10, 4))

        self.fig = plt.Figure(figsize=(6, 5.5), facecolor=BG_ROOT)
        self.fig.subplots_adjust(
            left=0.08, right=0.94, top=0.96, bottom=0.10, hspace=0.55
        )
    ...
    def _build_right(self, parent):
        ...
        tk.Label(ca, text="ALARMES", font=("Courier New", 7),
                 bg=BG_CARD, fg=TX_MUTED).pack()
        self.lbl_alarme = tk.Label(ca, text="0",
        ...
        tk.Label(cr, text="RESFR.", font=("Courier New", 7),
                 bg=BG_CARD, fg=TX_MUTED).pack()
        self.lbl_resf = tk.Label(cr, text="0",
                                  font=("Courier New", 22, "bold"),
                                  bg=BG_CARD, fg=COR_COLD)
        self.lbl_resf.pack()
    ...
    def _atualizar_historico(self):
        ...
        self.ax_hist.axhline(lim_max, color=COR_DANGER, linewidth=0.8,
                             linestyle="--", alpha=0.5)
        self.ax_hist.axhline(lim_min, color=COR_COLD,   linewidth=0.8,
                             linestyle="--", alpha=0.5)
        ...
        for i, (sid, pontos) in enumerate(sorted(por_sensor.items())):
            ...
            self.ax_hist.plot(ts_list, v_list,
                              color=cor, linewidth=1.4, alpha=0.9,
                              label=nome_cur)
            self.ax_hist.fill_between(ts_list, v_list,
                                      alpha=0.04, color=cor)

            # Card de resumo por sensor
            ...
            info.pack(side="left")
            tk.Label(info, text=nome_cur.upper(),
        ...
        self.ax_hist.xaxis.set_major_formatter(
            mdates.DateFormatter("%H:%M" if periodo != "tudo" else "%d/%m %H:%M")
        )
        ...
        if all_vals:
            self.ax_hist.set_ylim(
                min(min(all_vals), lim_min) - 3,
                max(max(all_vals), lim_max) + 4
            )

        self.ax_hist.legend(
            facecolor=BG_PANEL, labelcolor=TX_SECONDARY,
            fontsize=7, loc="upper left",
            framealpha=0.85, edgecolor=BORDER,
            ncol=min(len(por_sensor), 4),
    ...
    def _build_atuadores_aba(self, parent):
        ...
        tk.Label(toolbar, text="FILTRAR:",
                 font=("Courier New", 8), bg=BG_PANEL, fg=TX_SECONDARY
        ...
        self.listbox_atu = tk.Listbox(
            list_frame,
            bg=BG_CARD, fg=TX_SECONDARY,
            font=("Courier New", 8),
            selectbackground=BG_HOVER,
            selectforeground=TX_PRIMARY,
    ...
    def _atualizar_aba_atuadores(self):
        ...
        def parse_ts(a):
            try:
                return datetime.strptime(a[0]["timestamp"], "%Y-%m-%d %H:%M:%S")
            except:
                return datetime.now()

        if alarmes:
            ts_a = [parse_ts(a) for a in alarmes]
            vl_a = [a[1] for a in alarmes]
            self.ax_atu_line.scatter(ts_a, vl_a, color=COR_DANGER,
                                     s=18, zorder=4, label="Alarme", alpha=0.85)

        if resfrs:
            ts_r = [parse_ts(r) for r in resfrs]
            vl_r = [r[1] for r in resfrs]
            self.ax_atu_line.scatter(ts_r, vl_r, color=COR_COLD,
                                     s=18, zorder=4, label="Resfr.", alpha=0.85, marker="^")

        if alarmes or resfrs:
            self.ax_atu_line.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
            self.ax_atu_line.xaxis.set_major_locator(mdates.AutoDateLocator())
            plt.setp(self.ax_atu_line.get_xticklabels(),
                     rotation=0, ha="center", fontsize=7.5, color=TX_SECONDARY)
            self.ax_atu_line.legend(
                facecolor=BG_PANEL, labelcolor=TX_SECONDARY,
        ...
        for a in reversed(atuadores[-200:]):
            ...
            linha = f"{icone} {ts[-8:]}  {nome:<14} {valor}"
            self.listbox_atu.insert(0, linha)

        # Colore as linhas alternadas
        for i in range(self.listbox_atu.size()):
            item = self.listbox_atu.get(i)
            if item.startswith("⚠"):
                self.listbox_atu.itemconfig(i, fg=COR_DANGER)
            else:
                self.listbox_atu.itemconfig(i, fg=COR_COLD)

    # ═════════════════════════════════════════════════════════════════════════
    # STATUS FIXO — só atualiza o valor, não recria os widgets
    # ═════════════════════════════════════════════════════════════════════════
    def _registrar_sensores(self, estado):
        ...
        for sid in sorted(novos):
            ...
            tk.Checkbutton(
                row, text=nome_curto, variable=var,
                bg=BG_PANEL, fg=TX_SECONDARY,
                selectcolor=BG_CARD,
                activebackground=BG_PANEL,
                activeforeground=TX_PRIMARY,
            ...
            # Status row — criado UMA vez, nunca destruído
            lims = {"temperatura": (20, 33), "umidade": (45, 85)}
            nome_curto = sid.replace("sensor_", "").replace("_", " ")
    ...
    def _atualizar_atuadores(self, atuadores):
        ...
        total_a = sum(1 for a in atuadores
                      if isinstance(a, dict) and a.get("acao") == "ALARME")
        total_r = sum(1 for a in atuadores
                      if isinstance(a, dict) and a.get("acao") == "RESFRIAMENTO")
        ...
        if contagem:
            ...
            bars_a = self.ax_atu.bar(
                [i - w/2 for i in x], alarmes, w,
                color=COR_DANGER, label="Alarme", zorder=3, linewidth=0
            )
            bars_r = self.ax_atu.bar(
                [i + w/2 for i in x], resfr, w,
            ...
            for bar in list(bars_a) + list(bars_r):
                ...
                if h > 0:
                    self.ax_atu.text(
                        ...
                        str(int(h)),
                        ha="center", va="bottom",
                        color=TX_SECONDARY, fontsize=6.5
                    )
            nomes = [
                s.replace("sensor_", "")
                 .replace("temp_", "T")
            ...
            self.ax_atu.legend(
                facecolor=BG_CARD, labelcolor=TX_SECONDARY,
                fontsize=6.5, framealpha=0.9,
                edgecolor=BORDER, loc="upper right"
            )
            self.ax_atu.yaxis.set_major_locator(
                ticker.MaxNLocator(integer=True, nbins=4)
            )
        else:
            self.ax_atu.text(
                0.5, 0.5, "sem acionamentos",
                transform=self.ax_atu.transAxes,
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # FILTRAGEM / PLOTAGEM TEMPO REAL
    # ═════════════════════════════════════════════════════════════════════════
    def _agregar(self, pontos, max_pts=80):
        ...
        for i in range(0, len(pontos) - fator + 1, fator):
            ...
            hora  = grupo[len(grupo) // 2][0]
            media = round(sum(p[1] for p in grupo) / len(grupo), 1)
            res.append((hora, media))
        return res

    def _filtrar(self, historico):
        if not isinstance(historico, list):
            historico = []
        ativos   = {sid for sid, var in self._sensores_vars.items() if var.get()}
        segundos = PERIODOS.get(self._periodo_var.get(), 30)
        temp     = defaultdict(list)
        umid     = defaultdict(list)
        ...
        for entry in historico:
            ...
            hora  = ts.strftime("%H:%M:%S") if segundos <= 60 else ts.strftime("%H:%M")
            tipo  = entry.get("tipo", "")
            valor = entry.get("valor")
            if valor is None:
                continue
            if tipo == "temperatura":
                temp[sid].append((hora, float(valor)))
            elif tipo == "umidade":
                umid[sid].append((hora, float(valor)))

        return (
            {s: self._agregar(p) for s, p in temp.items()},
            {s: self._agregar(p) for s, p in umid.items()}
        )

    def _plotar_eixo(self, ax, dados, tipo_key):
        ...
        ax.axhline(lim_max, color=COR_DANGER, linewidth=0.7,
                   linestyle="--", alpha=0.5, zorder=1)
        ax.axhline(lim_min, color=COR_COLD,   linewidth=0.7,
                   linestyle="--", alpha=0.5, zorder=1)
        ...
        for i, (sid, pontos) in enumerate(dados.items()):
            if not pontos:
                continue
            horas   = [p[0] for p in pontos]
            valores = [p[1] for p in pontos]
            all_vals.extend(valores)
            cor     = CORES_GRAF[i % len(CORES_GRAF)]
            nome_cur = sid.replace("sensor_", "").replace("_", " ")

            intervalo_marker = max(1, len(pontos) // 12)
            ax.plot(horas, valores, color=cor, linewidth=1.6,
                    marker="o", markersize=2.5,
                    markevery=intervalo_marker,
                    label=nome_cur, zorder=3, alpha=0.92)
            ax.fill_between(horas, valores, alpha=0.05, color=cor, zorder=1)

            for hora, val in zip(horas, valores):
                if val > lim_max or val < lim_min:
                    ax.plot(hora, val, "o", color=COR_DANGER,
                            markersize=4, zorder=5, markeredgewidth=0, alpha=0.8)
            if pontos:
                ax.annotate(
                    f"{valores[-1]:.1f}{unidade}",
                    xy=(horas[-1], valores[-1]),
                    xytext=(5, 0), textcoords="offset points",
                    color=cor, fontsize=6.5, va="center", alpha=0.9, zorder=6
                )
        ...
        if n > 6:
            ...
            for j, lbl in enumerate(xticks):
                lbl.set_visible(j % step == 0)
        plt.setp(ax.get_xticklabels(),
                 rotation=0, ha="center",
                 fontsize=7.5, color=TX_SECONDARY)

    def _redesenhar(self):
        self._plotar(self._ultimo_historico)

    def _plotar(self, historico):
        self._ultimo_historico = (
            [e for e in historico if isinstance(e, dict)]
            if isinstance(historico, list) else []
        )
        temp, umid = self._filtrar(self._ultimo_historico)
    ...
    def _atualizar(self):
        def tarefa():
            ...
            self.after(0, self._registrar_sensores,  estado)
            self.after(0, self._atualizar_status,    estado)
            self.after(0, self._plotar,              historico)
            self.after(0, self._atualizar_atuadores, atuadores)
            self.after(0, self.status_bar.config, {
                "text": (
                    f"  atualizado {datetime.now().strftime('%H:%M:%S')} · "
                    f"período {self._periodo_var.get()} · "
                    f"{len(historico)} registros · "
                    f"{'online' if estado else 'sem conexão'}"
                )
            })
